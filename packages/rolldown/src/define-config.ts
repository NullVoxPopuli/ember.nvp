 ...
 * logging) live on the `ember()` meta-plugin's `emberConfig()` step, so they
 * apply as long as `ember()` is in `plugins` — whether you use this helper,
 * tsdown's `defineConfig`, or none at all.
 *
 * ```js
